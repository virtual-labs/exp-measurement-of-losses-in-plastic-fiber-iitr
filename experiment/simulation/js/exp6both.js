...
var rand=(Math.random() * (1 - 0.3) + 0.3).toFixed(3);;
...
function bthdhlfrc() {
    ...
    if (bttn == "Power:Off") {alert("Please turn on to draw the graph");}
	else{
...
//---------------------------------------------------------Sine wave-------------------------------------------------------------------------------//
    ...
    for (var j = 0; j < axes.N; j++) {
        x1[j] = tstart1 + j * dt1;
        y1[j] = (vp-rand) * Math.sin(2 * 3.1415 * frqfng * x1[j] + phsl * 3.1415 / 180);
    }
